...
```mermaid
flowchart TD
    Start([流量]) --> Special{专属 IP /<br/>邮箱 / 币安?}
    Special -->|命中| Direct[DIRECT 直连]
    Special -->|mail.com| US[美国自动]
    Special -->|否| Personal{私有 /<br/>自定义列表?}
    Personal -->|direct.list / 私有| Direct
    Personal -->|default-proxy.list| Custom[自用分组]
    Personal -->|否| Services{服务<br/>规则集?}
    Services -->|Apple / AI / GitHub / Google /<br/>YouTube / Telegram / Netflix / ...| Proxy[对应服务分组]
    Services -->|否| CN{国内<br/>域名 / IP?}
    CN -->|是| Direct
    CN -->|否| Ad{广告域名?}
    Ad -->|是| Reject[REJECT 拦截]
    Ad -->|否| Final[漏网之鱼 → 默认代理]
```
